...
`)}
${t}<span class="json-syntax-punctuation">}</span>`}return String(s)}function kn(s,e){let t={fields:[]};e&&e.trim()&&(t.title=e.trim());let r=Array.from(s.entries()).filter(([,n])=>n.checked).sort((n,i)=>n[1].order-i[1].order);return t.fields=r.map(([n,i])=>({path:n,title:i.title||n})),t}function jt(s,e){return`"${JSON.stringify(kn(s,e)).replace(/"/g,'\\"')}"`}function Ur(s){return s?s.split(`
...
`)}function An(s,e){return s?Object.entries(s).filter(([t])=>t!=="value"&&!e.has(t)).map(([t,r])=>({name:t,default:r.defaultValue,config:r})):[]}function zr(s,e,t){let r=An(s,t);return b`
      ...
      <tbody>
        ${r.map(n=>b`
          <tr>
            ...
            <td class="value-current">${Cn(n.name,n.config,e)}</td>
          </tr>
        `)}
      </tbody>
    </table>
  `}function Cn(s,e,t){let r=t[s],n=e.type;if(n==="boolean")return b`
    ...
    `}return n==="number"||n==="integer"?b`<span style="font-weight: 500;">${r}</span>`:b`<span>${Pt(r)}</span>`}function Hr(s,e){let t=a=>b`
  ...
  `,r=s.oauthTokenResponse?JSON.stringify(s.oauthTokenResponse,null,2):"",n=s.requestHeaders?pt(s.requestHeaders):"",i=s.requestBody?pt(s.requestBody):"",o=s.apiResponse?pt(s.apiResponse):"";return b`
    <table class="debug-table">
      <thead>
