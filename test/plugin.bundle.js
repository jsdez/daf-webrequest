...
`)}
${t}<span class="json-syntax-punctuation">}</span>`}return String(s)}function Cn(s,e){let t={fields:[]};e&&e.trim()&&(t.title=e.trim());let r=Array.from(s.entries()).filter(([,n])=>n.checked).sort((n,i)=>n[1].order-i[1].order);return t.fields=r.map(([n,i])=>({path:n,title:i.title||n})),t}function jt(s,e){return`"${JSON.stringify(Cn(s,e)).replace(/"/g,'\\"')}"`}function Ur(s){return s?s.split(`
...
`)}function $n(s,e){return s?Object.entries(s).filter(([t])=>t!=="value"&&!e.has(t)).map(([t,r])=>({name:t,default:r.defaultValue,config:r})):[]}function zr(s,e,t){let r=$n(s,t);return b`
      ...
      <tbody>
        ${r.map(n=>b`
          <tr>
            ...
            <td class="value-current">${En(n.name,n.config,e)}</td>
          </tr>
        `)}
      </tbody>
    </table>
  `}function En(s,e,t){let r=t[s],n=e.type;if(n==="boolean")return b`
  ...
  `,r=s.oauthTokenResponse?JSON.stringify(s.oauthTokenResponse,null,2):"",n=s.requestHeaders?ht(s.requestHeaders):"",i=s.requestBody?ht(s.requestBody):"",o=s.apiResponse?ht(s.apiResponse):"";return b`
    <table class="debug-table">
      <thead>
