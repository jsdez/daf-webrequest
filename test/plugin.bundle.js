...
`)}
${t}<span class="json-syntax-punctuation">}</span>`}return String(s)}function An(s,e){let t={fields:[]};e&&e.trim()&&(t.title=e.trim());let r=Array.from(s.entries()).filter(([,n])=>n.checked).sort((n,i)=>n[1].order-i[1].order);return t.fields=r.map(([n,i])=>({path:n,title:i.title||n})),t}function jt(s,e){return`"${JSON.stringify(An(s,e)).replace(/"/g,'\\"')}"`}function Ur(s){return s?s.split(`
...
`)}function Cn(s,e){return s?Object.entries(s).filter(([t])=>t!=="value"&&!e.has(t)).map(([t,r])=>({name:t,default:r.defaultValue,config:r})):[]}function zr(s,e,t){let r=Cn(s,t);return b`
      ...
      <tbody>
        ${r.map(n=>b`
          <tr>
            ...
            <td class="value-current">${$n(n.name,n.config,e)}</td>
          </tr>
        `)}
      </tbody>
    </table>
  `}function $n(s,e,t){let r=t[s],n=e.type;if(n==="boolean")return b`
