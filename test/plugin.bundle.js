...
`)}function En(s,e){return s?Object.entries(s).filter(([t])=>t!=="value"&&!e.has(t)).map(([t,r])=>({name:t,default:r.defaultValue,config:r})):[]}function Fr(s,e,t){let r=En(s,t);return b`
  ...
  `}function In(s,e,t){let r=t[s],n=e.type;if(n==="boolean")return b`
    ...
    `}return n==="number"||n==="integer"?b`<span style="font-weight: 500;">${r}</span>`:b`<span>${Pt(r)}</span>`}function Ur(s,e){let t=a=>b`
  ...
  `}function zr(s){if(!s.trim())return"";let e="",t="",r="";try{let n=JSON.parse(s);e=JSON.stringify(n),t=`"${e.replace(/\\/g,"\\\\").replace(/"/g,'\\"')}"`}catch(n){r=n.message}return b`
  ...
  `}function Hr(s){if(!s.trim()||!G(s))return"";try{let e=JSON.parse(s);return b`
      <div class="form-group">
...
`)}
${t}<span class="json-syntax-punctuation">}</span>`}return String(s)}function Dr(s,e){let t=G(s),r=jr(s);return b`
