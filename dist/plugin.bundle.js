...
`)}updated(e){e.has("value")&&(console.log("[Updated Lifecycle] Value property changed, dispatching ntx-value-change event"),this.dispatchEvent(new CustomEvent("ntx-value-change",{detail:this.value,bubbles:!0,composed:!0}))),e.has("sendAPICall")&&this.sendAPICall&&this.handleAPICallTrigger(),e.has("submitHidden")&&this.toggleSubmitButtonVisibility()}toggleSubmitButtonVisibility(){let e="ntx-submit-button-hidden-style";if(this.submitHidden){if(!document.getElementById(e)){let s=document.createElement("style");s.id=e,s.textContent=`
    ...
    `}renderAvailableFields(e,s){let o=[],i=(r,a)=>{r&&typeof r=="object"&&!Array.isArray(r)&&Object.keys(r).forEach(l=>{var u;let d=a?`${a}.${l}`:l,c=r[l];if(c!==null&&typeof c!="object"){let g=d,y=((u=this.formatterSelectedFields.get(g))==null?void 0:u.checked)||!1;o.push(p`
      ...
      <div 
        ...
        @drop=${r=>{r.preventDefault();let a=parseInt(r.dataTransfer.getData("text/plain")),l=i;if(a!==l){let d=Array.from(e),[c]=d.splice(a,1);d.splice(l,0,c),d.forEach(([u,g],y)=>{this.formatterSelectedFields.set(u,F($({},g),{order:y}))}),this.requestUpdate()}}}
        style="
          display: flex;
          align-items: center;
          gap: 10px;
